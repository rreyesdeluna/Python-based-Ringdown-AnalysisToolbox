...
August 11th, 2021
	...
	def click_btn32(self):
		...
		if self.Check_Data(2, 34, 56, 7, 9) == True:
			...
			if self.val_opmn3.get() == 'Single-Channel':
				...
				for c in range(can):
					...
					if self.val_opmn2.get() == 'Prony':
						modes, mag, ang, damp, freq, damprat, enrgy, roots = self.Prony(Y, N, dt, fct_method)

					if self.val_opmn2.get() == 'Matrix Pencil':
						modes, mag, ang, damp, freq, damprat, enrgy, roots = self.MatrixPencil(Y, N, dt, fct_method)

					if self.val_opmn2.get() == 'ERA':
						modes, mag, ang, damp, freq, damprat, enrgy, roots = self.ERA(Y, N, dt, fct_method)
			...
			if self.val_opmn3.get() == 'Multi-Channel':
				...
				Y = y_vec
				if self.val_opmn2.get() == 'ERA':
					modes, mag, ang, damp, freq, damprat, enrgy, roots = self.MultiERA(Y, N, dt, fct_method)
	...
	def Fig_Roots(self):
		...
		for r in r_aprx:
			...
			ax.plot(r[0].real, -r[0].imag, linestyle = 'None', color = colors(i_signal), marker = mrk)
			ax.annotate(' ' + str(r[1]), (r[0].real, +r[0].imag), fontsize = 17)
			ax.annotate(' ' + str(r[1]), (r[0].real, -r[0].imag), fontsize = 17)

		ax.axis('equal')
		ax.set_title('Pole-Zero')
		ax.set_xlabel('Re')
		ax.set_ylabel('Im')
	...
	def Fig_ModalShape(self):
		...
		self.fig.suptitle('Mode Shapes')
		self.fig.legend(handles, labels, loc = 'lower center', bbox_to_anchor = (0.5, 0.05), fancybox = True, ncol = can, fontsize=18)
	...
	def MatrixPencil(self, Y, N, dt, fct_method):
	...
	def ERA(self, Y, N, dt, fct_method):
	...
	def MultiERA(self, Y, N, dt, fct_method):
	...
	def Get_Data(self, file_path):
		...
		# 				y[index].append(float(jData))
				...
				for indx, idata in enumerate(Data[1:]):

					y.append([])
					# t.append([])

					for jndx, jdata in enumerate(idata):

						if jndx == 0: t.append(float(jdata))
						if jndx > 0:
							if jdata == '': y[indx].append(np.nan)
							if jdata != '': y[indx].append(float(jdata))
	...
	def Get_data(self, file_path):
		...
		for indx, idata in enumerate(Data[1:]):
			...
			for jndx, jdata in enumerate(idata):
				...
				if jndx == 0: t[indx].append(float(jdata))
				if jndx > 0:
					if jdata == '': y[indx].append(np.nan)
					if jdata != '': y[indx].append(float(jdata))
